...
namespace PurchaseOrder
{
    ...
    "ProductName";
		...
		public void SubmitOrders()
		{
			...
			//TODO 1: Update the database using the Update method of OrdersDataAdapter
            //and OrderDetailsDataAdapter. The Cascading Update rule enforced by the 
            //Relation between the two tables should automatically retrieve the OrderID 
            //generated by the server when an Order row is created assign it
            //to related rows in the Order Details table on the client.
			try
			{
                OrdersDataAdapter.Update(ordersTable);
                OrderDetailsDataAdapter.Update(orderDetailsTable);
			}
			catch(Exception xcp)
			{
